...
async def build_entry_chart(
    ...
    Args:
        ...
        bars: Number of 5m candles to show (default 60 = 5h window).
        interval: MEXC kline interval string.
    ...
    try:
        ...
        if df is None or df.empty:
            ...
            return None
        df = df.iloc[-bars:]
        return _render(df, symbol, side, entry_price, tp_price, sl_price)
    except Exception as exc:
...
def _render(
    ...
    title = f"{symbol}  5m  —  {arrow}  entry {entry:,.1f}"

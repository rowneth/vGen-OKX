...
class LiveVolumeExecutor:
	...
	max_live_trades: int = 1_000_000  # effectively uncapped — set low only for debug runs
	max_notional_usd: float = 5000.0  # high cap — session controls real sizing
	post_only_fill_timeout: float = 45.0
	...
	max_reprice_attempts: int = 3  # cap aggressively — block-the-next-bar is worse than missing one fill
	...
	# How many session entries we've blocked because the prior trade is still
	# open. Notify the operator after the first block and then every K more.
	_blocked_by_open_count: int = field(default=0, init=False)
	_blocked_notify_every: int = field(default=5, init=False)
	...
	async def handle_entry(self, payload: Dict[str, Any]) -> None:
		...
		async with self._lock:
			...
			if self._current is not None and not self._current.closed:
				self._blocked_by_open_count += 1
				n = self._blocked_by_open_count
				LOGGER.warning(
					"live entry skipped — prior trade still open (oid=%s, blocked=%d)",
					self._current.external_oid, n,
				)
				# Notify on the 1st block and then every Kth block so the operator
				# knows the silence is the prior live trade not closing on MEXC,
				# not a dead bot or boring market.
				if n == 1 or (n % max(self._blocked_notify_every, 1) == 0):
					oid = self._current.external_oid
					side = self._current.side.upper()
					entry = self._current.fill_price or self._current.entry_price_req
					try:
						await self._notify(
							f"⏳ *Live entry blocked — prior trade still open*\n"
							f"Side `{side}`  entry `{entry:,.1f}`\n"
							f"Skipped session entries: `{n}`\n"
							f"Waiting for MEXC TP/SL to close oid `{oid}`\\."
						)
					except Exception:  # noqa: BLE001
						pass
				return
			filled_count = sum(1 for t in self.trades if t.filled)
			if filled_count >= self.max_live_trades:
				# Soft skip — do NOT permanently disable. Operator may have set a low
				# cap intentionally for a debug session; we still want the bot alive.
				LOGGER.info(
					"live entry skipped — filled=%d >= max_live_trades=%d (soft cap)",
					filled_count, self.max_live_trades,
				)
				return
				...
				if isinstance(order, dict):
					...
					if state == 4:
						...
						if record.reprice_attempts >= self.max_reprice_attempts:
							...
							# A reprice cancel can lose the race with a MEXC fill: the
							# prior order fills just as our cancel arrives, then the new
							# order is rejected because the position already exists.
							# Check open_positions before declaring failure.
							try:
								open_pos = await self.client.get_open_positions(symbol=self.symbol)
								items = open_pos if isinstance(open_pos, list) else (
									open_pos.get("data")
									if isinstance(open_pos, dict) and isinstance(open_pos.get("data"), list)
									else []
								)
								our_code = 1 if record.side == "long" else 2
								for pos in items or []:
									if not isinstance(pos, dict):
										continue
									if pos.get("symbol") != self.symbol:
										continue
									if int(pos.get("positionType", pos.get("position_type", 0))) != our_code:
										continue
									st = int(_as_float(pos.get("state", 0)))
									hold_vol = _as_float(pos.get("holdVol", pos.get("hold_vol", 0)))
									if st in (1, 2) and hold_vol > 0:
										fill_px = (
											_as_float(pos.get("openAvgPrice") or pos.get("open_avg_price"))
											or record.entry_price_req
										)
										LOGGER.warning(
											"LIVE orphan position detected after max reprices "
											"oid=%s fill_price=%.2f — adopting",
											record.external_oid, fill_px,
										)
										record.filled = True
										record.fill_price = fill_px
										self._current = record
										await self._notify(
											f"⚠️ *LIVE orphan position adopted* \\(reprice race\\)\n"
											f"Side `{record.side.upper()}`  fill ≈ `{fill_px:,.1f}`\n"
											f"Watching for MEXC close\\.\\.\\."
										)
										asyncio.create_task(self._watch_position_close(record))
										return
							except Exception as _orp_exc:  # noqa: BLE001
								LOGGER.warning("orphan position check failed: %s", _orp_exc)
							record.closed = True
	...
	async def _watch_position_close(self, record: LiveTradeRecord) -> None:
		...
		while time.time() - start < self.position_watch_timeout:
			...
			# Find our position: matching symbol + side + state.
			# MEXC position state: 1 = holding (open), 2 = system-held, 3 = closed.
			# We treat state in (1, 2) as still open. We also match on positionType
			# (1=long, 2=short).
			...
			for pos in items or []:
				...
				hold_vol = _as_float(pos.get("holdVol", pos.get("hold_vol", 0)))
				# Open if state says holding AND there are contracts held.
				# (A fully-closed position on MEXC reports state=3; an open one
				# reports state=1 or 2 with holdVol > 0.)

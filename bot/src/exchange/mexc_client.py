...
class MEXCClient:
	...
	async def get_history_positions(
		...
		"""Fetch historical (closed) futures positions with realized PnL.

		Each record includes: positionType (1=long, 2=short), openAvgPrice,
		closeAvgPrice, realised (realized PnL), fee, openTime, closeTime,
		closeVol, leverage, exitType (1=TP, 2=SL, 3=liquidation, 4=manual).

		Args:
			symbol: Optional contract symbol filter (e.g. ``"BTC_USDT"``).
			start_time: Optional start filter in milliseconds.
			end_time: Optional end filter in milliseconds.
			page_num: Page number (1-indexed).
			page_size: Results per page (max 100).

		Returns:
			Decoded positions history payload.
		"""
		params: Dict[str, Any] = {"page_num": page_num, "page_size": page_size}
		if symbol is not None:
			params["symbol"] = symbol
		...
		if end_time is not None:
			params["end_time"] = end_time
		return await self._request(
			"GET", "/api/v1/private/position/list/history_positions", params=params, auth=True
		)
